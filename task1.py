...
def getLink(pageurl, find_name):
    ...
    try:
        if len(relation) == 0 | len(name) == 0:
            ...
            for link in bs.find('div', {'class': 'star-info-block relations'}).find_all('a',                                                                         href=re.compile('^(/item/)')):
                if 'href' in link.attrs:
                    if link.attrs['href'] not in pages:
                        newpage = link.attrs['href']
                        url1.append("https://baike.baidu.com" + newpage)
...
def main():
    ...
    print(get_)
    relation, name, url1 = getLink(urllib.parse.quote(get_), find_name)
